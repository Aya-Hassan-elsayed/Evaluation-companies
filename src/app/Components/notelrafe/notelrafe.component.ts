...
@Component({
  selector: 'app-notelrafe',
  templateUrl: './notelrafe.component.html',
  styleUrl: './notelrafe.component.css'
})
export class NOTelrafeComponent implements OnInit {
 ...
 constructor(private NotuploadServices:TaqimService,private spinner:NgxSpinnerService , private dialog:MatDialog){
  ...
  applyRequestTypeFilter(requestType: string): void {
    ...
    this.getNotUPloadedCompanies(requestType)
  }
  ...
  openAcceptedPercentageDialog(): void {
    this.dialog.open(ChartDialogNOTuploadComponent, {
      width: '80%',
